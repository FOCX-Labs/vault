...
export class VaultAdminOperations {
  ...
  async updateVaultConfig(params: {
    ...
    try {
      ...
      if (params.unstakeLockupPeriod !== undefined) {
        updateParams.unstakeLockupPeriod = new anchor.BN(params.unstakeLockupPeriod * 60 * 60); // convert hours to seconds
        console.log(`📝 Unstake lockup period: ${params.unstakeLockupPeriod} hours`);
      }
      
      if (params.managementFee !== undefined) {
        updateParams.managementFee = new anchor.BN(params.managementFee);
        console.log(`📝 Management fee: ${params.managementFee} basis points (${params.managementFee / 100}%)`);
      }
      
      if (params.minStakeAmount !== undefined) {
        updateParams.minStakeAmount = new anchor.BN(params.minStakeAmount * 1e6); // convert to 6 decimals
      ...
      if (params.maxTotalAssets !== undefined) {
        if (params.maxTotalAssets === null) {
          ...
          updateParams.maxTotalAssets = new anchor.BN(params.maxTotalAssets * 1e6); // convert to 6 decimals
  ...
  async getVaultInfo(): Promise<any> {
    try {
      ...
      console.log(`Name: ${Buffer.from(vaultAccount.name).toString().replace(/\0/g, '')}`);
      console.log(`Owner: ${vaultAccount.owner.toString()}`);
      console.log(`Platform account: ${vaultAccount.platformAccount.toString()}`);
      console.log(`Total assets: ${Number(vaultAccount.totalAssets.toString()) / 1e6} USDC`);
      console.log(`Total shares: ${vaultAccount.totalShares.toString()}`);
      console.log(`Management fee: ${Number(vaultAccount.managementFee.toString())} basis points (${Number(vaultAccount.managementFee.toString()) / 100}%)`);
      console.log(`Minimum stake amount: ${Number(vaultAccount.minStakeAmount.toString()) / 1e6} USDC`);
      console.log(`Maximum total assets: ${vaultAccount.maxTotalAssets.toString() === "0" ? "Unlimited" : (Number(vaultAccount.maxTotalAssets.toString()) / 1e6) + " USDC"}`);
      console.log(`Unstake lockup period: ${Number(vaultAccount.unstakeLockupPeriod.toString()) / 3600} hours`);
      console.log(`Is paused: ${vaultAccount.isPaused}`);
      console.log(`Created at: ${new Date(Number(vaultAccount.createdAt.toString()) * 1000).toLocaleString()}`);
...
async function loadAdminConfig() {
  const config: VaultAdminConfig = {
    ...
    rpcUrl: getOption("--rpc", "https://api.devnet.solana.com") || "https://api.devnet.solana.com"
  };
    ...
    switch (command) {
      ...
      case "update-lockup":
        ...
        if (lockupHours < 10/60) {
          throw new Error(`❌ Lockup period must be at least 10 minutes (${(10/60).toFixed(3)} hours). You provided: ${lockupHours} hours`);
        }
        // Validate maximum 90 days as per contract
        if (lockupHours > 90 * 24) {
          throw new Error(`❌ Lockup period must not exceed ${90 * 24} hours (90 days). You provided: ${lockupHours} hours`);
        }
        console.log(`⏰ Updating unstake lockup period to ${lockupHours} hours...`);
      ...
      case "update-lockup-min":
        ...
        if (lockupMinutes < 10) {
          throw new Error(`❌ Lockup period must be at least 10 minutes. You provided: ${lockupMinutes} minutes`);
        }
        // Validate maximum 90 days (129600 minutes) as per contract
        if (lockupMinutes > 90 * 24 * 60) {
          throw new Error(`❌ Lockup period must not exceed ${90 * 24 * 60} minutes (90 days). You provided: ${lockupMinutes} minutes`);
        }
        console.log(`⏰ Updating unstake lockup period to ${lockupMinutes} minutes (${lockupHoursFromMin.toFixed(3)} hours)...`);
        await operations.updateVaultConfig({ unstakeLockupPeriod: lockupHoursFromMin });
        break;

      case "update-fee":
        const feeBasisPoints = parseInt(args[1]);
        if (isNaN(feeBasisPoints) || feeBasisPoints < 0) {
          throw new Error("Please provide a valid management fee in basis points (e.g., 100 = 1%)");
        }
        console.log(`💰 Updating management fee to ${feeBasisPoints} basis points (${feeBasisPoints / 100}%)...`);
      ...
      case "update-min-stake":
        ...
        console.log(`💵 Updating minimum stake amount to ${minStakeAmount} USDC...`);
      ...
      case "update-max-assets":
        ...
        if (!maxAssetsInput) {
          throw new Error("Please provide maximum total assets amount in USDC or 'unlimited'");
        }
        ...
        if (maxAssetsInput.toLowerCase() === "unlimited") {
          ...
          if (isNaN(maxAssetsAmount) || maxAssetsAmount <= 0) {
            throw new Error("Please provide a valid maximum total assets amount in USDC or 'unlimited'");
          }
          maxTotalAssets = maxAssetsAmount;
          console.log(`📈 Updating maximum total assets to ${maxAssetsAmount} USDC...`);
        }
      ...
      case "update-multiple":
        console.log("🔧 Interactive multiple parameter update not implemented yet.");
