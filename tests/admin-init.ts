...
import contract_info from '../client/contract_info.json'
...
describe('admin_initialization', () => {
  ...
  it('Initialize vault contract (Admin Only)', async () => {
    ...
    if (owner.publicKey.toString() === expectedAddress) {
      console.log(
        `✅ Wallet address verification successful: ${expectedAddress}`
      )
    } else {
    ...
    try {
      ...
      console.log(
        `✅ Reward source account (regular): ${rewardSourceAccount.toString()}`
      )
    }
    ...
    console.log(
      `📝 Note: rewardsTokenAccount should maintain a balance of 0, this is normal`
    )
    ...
    console.log(
      `   - Platform account: ${contract_info.platform_account} (receives 50% of rewards)`
    )
    console.log(
      `   - Unstake lockup period: ${contract_info.unstake_lockup_period} hours`
    )
    console.log(
    ...
    await program.methods
      .initializeVault({
        ...
        unstakeLockupPeriod: new anchor.BN(
    ...
    console.log(
      `   ✅ Management Fee: ${vault.managementFee.toString()} basis points (${
        vault.managementFee.toNumber() / 100
      }%)`
    )
    console.log(
    ...
    console.log(
      `   ✅ Lockup Period: ${
        vault.unstakeLockupPeriod.toNumber() / 3600
      } hours`
    )
    console.log(`   ✅ Is Paused: ${vault.isPaused}`)
    console.log(
      `   ✅ Created At: ${new Date(
        vault.createdAt.toNumber() * 1000
      ).toISOString()}`
    )
    ...
    console.log(`💸 Reward source account: ${rewardSourceAccount.toString()}`)
    console.log(
      `💼 Management fee: ${vault.managementFee.toNumber() / 100}% annualized`
    )
    console.log(
      `⏰ Unstake lockup period: ${
        vault.unstakeLockupPeriod.toNumber() / 86400
      } days`
    )
    console.log(
